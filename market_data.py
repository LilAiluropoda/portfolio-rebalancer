...
class AlpacaOptionData(OptionQuoteSource):
    ...
    def _parseSnapshot(self, symbol: str, raw: dict) -> OptionSnapshot | None:
        ...
        if not bid or bid <= 0 or not ask or ask <= 0:
            _logger.warning("[Option Valuation] Skipping %s: non-positive quote (bid=%s)", symbol, bid)
            return None
        ...
        try:
            ...
            _logger.warning("[Option Valuation] Skipping %s: missing or unparseable quote timestamp (%r)", symbol, rawTs)
            return None
        ...
        if quoteAge > timedelta(days=_MAX_QUOTE_AGE_DAYS) or quoteAge < timedelta(hours=-1):
            _logger.warning(
                "[Option Valuation] Skipping %s: quote timestamp %s is older than %s days",
                symbol,
                quote["t"],
                _MAX_QUOTE_AGE_DAYS,
            )
        ...
        if delta is None:
            _logger.warning(
                "[Option Valuation] Skipping %s: greeks.delta is null (no fallback estimation)", symbol
            )
            return None
